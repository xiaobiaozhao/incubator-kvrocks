...
Status SlotMigrate::SendSnapshot(void) {
  ...
  for (iter->Seek(prefix); iter->Valid(); iter->Next()) {
    ...
    auto stat = MigrateOneKey(rocksdb::Slice(user_key),
                              iter->value().ToString(), &restore_cmds);
    if (stat.IsOK()) {
...
// bool SlotMigrate::GetSlotKeyMetadata(const rocksdb::Slice &prefix_key,
// std::string *bytes) {
//   rocksdb::ReadOptions read_options;
//   read_options.snapshot = slot_snapshot_;
//   auto s = storage_->GetDB()->Get(read_options,
//   storage_->GetCFHandle("metadata"), prefix_key, bytes); if (!s.ok()) {
//     LOG(ERROR) << "[migrate] Failed to get metadata of key: " <<
//     prefix_key.ToString()
//       << ", Err: " << s.ToString();
//     return false;
//   }
...
Status SlotMigrate::MigrateOneKey(rocksdb::Slice key, const std::string &val,
                                  std::string *restore_cmds) {
  std::string prefix_key;
  AppendNamespacePrefix(key, &prefix_key);
  std::string bytes = val;
  // bool st = GetSlotKeyMetadata(prefix_key, &bytes);
  // if (!st) return Status(Status::NotOK, "[migrate] Failed to get key's
  // metadata");
